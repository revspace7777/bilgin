#!/usr/bin/env python3
"""
Improved Tandemovers.com Website Scraper
...
class TandemoversImprovedScraper:
    def __init__(self):
    ...
    def extract_main_content(self, soup):
        ...
        # Find main content areas with improved selectors
        main_content = []
        
        # Look for Fusion Builder content (common in WordPress themes)
        fusion_content = soup.find_all(class_=re.compile(r'fusion-text|fusion-builder|fusion-column'))
        if fusion_content:
            main_content.extend(fusion_content)
        ...
        for selector in main_selectors:
            ...
            for elem in elements:
                if elem not in main_content:
                    main_content.append(elem)
        
        # If still no content, look for paragraphs and headings with substantial text
        if not main_content:
...
if __name__ == "__main__":
    scraper = TandemoversImprovedScraper()
